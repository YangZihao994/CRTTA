...
def apply_cv2_motion_blur(img, radius, sigma, angle):
    """使用 OpenCV 模拟 ImageMagick 的方向性运动模糊"""
    size = int(radius)
    if size == 0:
...
def motion_blur(x, severity=1):
    ...
    # 采用替换后的 OpenCV 运动模糊
    blurred = apply_cv2_motion_blur(x_np, radius=c[0], sigma=c[1], angle=angle)

    if blurred.ndim == 2:  # 如果是单通道灰度图，转为 RGB 三通道
        return np.clip(np.stack([blurred, blurred, blurred], axis=-1), 0, 255)
    else:
...
def snow(x, severity=1):
    ...
    # 使用自定义的 OpenCV 运动模糊
    blurred_snow = apply_cv2_motion_blur(snow_layer_uint8, radius=c[4], sigma=c[5], angle=angle)
    
    snow_layer = blurred_snow.astype(np.float32) / 255.
